  ...
  ({
    ...
    const elements = useMemo(() => {
      ...
      if (splitBy === "characters") {
        return words.map((w, i) => ({ characters: splitIntoCharacters(w), needsSpace: i !== words.length - 1 }));
      }
    ...
    const getStaggerDelay = useCallback((index: number) => {
      const total = splitBy === "characters"
        ? (elements as WordObject[]).reduce((a, w) => a + w.characters.length + (w.needsSpace ? 1 : 0), 0)
    ...
    return (
      ...
      >
        ...
        {(splitBy === "characters"
          ...
          : (elements as string[]).map((el, i) => ({ characters: [el], needsSpace: i !== (elements as string[]).length - 1 }))
        ).map((wordObj, wordIndex, array) => {
          const previousCharsCount = array.slice(0, wordIndex).reduce((s, w) => s + w.characters.length, 0);
          return (
